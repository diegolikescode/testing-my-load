...
const pickRandChar = pickRandFactory([
  "A",
  "B",
  "C",
  "D",
...
function getInvalidNascimento() {
  const year =
    pickRandChar() + pickRandChar() + pickRandChar() + pickRandChar();
...
const pickRandStack = pickRandFactory([
  "Javascript",
  "Python",
  "Go",
  "Java",
...
function gerarPessoa() {
  return {
    ...
    apelido: itsMeantToBe(oneInOneHundred)
      ? getInvalidApelido()
      : getValidApelido(),
    nascimento: itsMeantToBe(oneInOneHundred)
      ? getInvalidNascimento()
      : getValidNascimento(),
...
function gerarTermoDeBusca() {
  ...
  return faker.person.firstName().slice(0, randBetween4And12()).trim();
}
